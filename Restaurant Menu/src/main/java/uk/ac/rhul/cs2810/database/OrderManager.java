 ...
 * 
 * @author Marley Dey, Daniel Rogerson
 *
 */
public class OrderManager {
  ...
  private String serialiseMenuItems(Map<MenuItem, Integer> orderedItems) {
    ...
    int i = 0;
    // For each item serialize the item in a string format
  ...
  private Map<MenuItem, Integer> deserialiseMenuItems(String orderedItemsString) {
    ...
    if (orderedItemsString == null || orderedItemsString.isEmpty()) {
      System.out.println("ERROR: Order found with no items, ignoring order!");
      return null;
    }
    ...
    for (String itemQuantity : splitItemQuantityString) {
      ...
      if (mi == null) {
        System.out.println("While attempting to deseirilise order items, an order contains "
            + "items that are no longer on the menu!");
        return null;
      }
  ...
  private List<Order> getAllOrdersWhere(String where) {
    ...
    ResultSet rs = dm.query(dm.createPreparedStatement(
        "SELECT * FROM `" + ORDERS_TABLE_NAME + "` WHERE " + where + " ORDER BY time_placed ASC"));
    try {
      ...
      while (rs.next()) {
        ...
        if (menuItemsText == null || menuItemsText.isEmpty()) {
          continue;
        }
  ...
  /** Deletes all the orders in the orders table. */
  public void deleteAllOrders() {

    dm.execute(dm.createPreparedStatement("TRUNCATE TABLE " + ORDERS_TABLE_NAME));
    System.out.println("Orders truncated from " + ORDERS_TABLE_NAME + " table");
  }
